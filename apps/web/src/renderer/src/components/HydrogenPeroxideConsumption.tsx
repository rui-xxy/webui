...
const generateData = (days: number) => {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    // Simulate H2O2 consumption (Liters or kg)
    const baseValue = 50 + Math.sin(i * 0.8) * 10;
    const randomFactor = Math.random() * 5 - 2.5;
    
    data.push({
      date: `${date.getMonth() + 1}/${date.getDate()}`,
      value: Math.round((baseValue + randomFactor) * 10) / 10,
    });
  }
  return data;
};

export const HydrogenPeroxideConsumption = () => {
      ...
      >
        <CardHeader className="flex justify-between items-start pb-0 px-5 pt-5">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              <div className="p-1.5 bg-secondary-100 text-secondary-600 rounded-lg">
                <FlaskConical size={18} />
              </div>
              <h4 className="font-bold text-base text-default-900">双氧水消耗</h4>
            </div>
            <p className="text-xs text-default-500">化学品用量监控</p>
          </div>
          <Chip
        ...
        <CardBody className="flex flex-col justify-end px-5 pb-5 pt-2 overflow-hidden">
          <div className="flex items-baseline gap-1 mb-2">
            ...
            <span className="text-xs text-default-500 font-medium">L</span>
          </div>
          ...
          <div className="h-[50px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data}>
                <defs>
                  <linearGradient id="colorPeroxide" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex gap-4">
                       <div className="flex flex-col">
                          ...
                          <span className="text-2xl font-bold text-default-900">{currentValue} L</span>
                       </div>
                       <div className="flex flex-col">
                          <span className="text-sm text-default-500">平均消耗</span>
                          <span className="text-2xl font-bold text-default-900">
                            {Math.round(data.reduce((acc, curr) => acc + curr.value, 0) / data.length)} L
                          </span>
                       </div>
                    </div>
                    <DateRangePicker 
                  ...
                  <div className="h-[400px] w-full bg-default-50 rounded-xl p-4 border border-default-100">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                        <defs>
                          <linearGradient id="colorPeroxideModal" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="5%" stopColor="#7828C8" stopOpacity={0.3}/>
                            <stop offset="95%" stopColor="#7828C8" stopOpacity={0}/>
                          </linearGradient>
                        </defs>
